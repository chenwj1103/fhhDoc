...
def detectRunningExec(jarname):
    ...
    command = "ps -ef | grep -E '%s' |grep -Ev 'grep|start-zmtbus.py|python' " % (jarname)
...
def updateNginxConfig(configpath, port):
...
server {
    listen       80;
    server_name local.Integration.fhh.ifengidc.com;
    location / {
        proxy_pass http://zmt-bus;
...
server {
    listen       80;
    server_name data.vampire.ifeng.com;
    location / {
        proxy_pass http://zmt-bus;
        proxy_set_header Connection "Keep-Alive";
        proxy_set_header Proxy-Connection "Keep-Alive";
...
def chooseNewPort(startport, ports):
    ...
    for i in range(startport, startport + 1000):
...
if __name__ == '__main__':
    ...
    # nginx
    port = newPorts["zmt-bus"]
    updateNginxConfig(args.nginxconf, port)
