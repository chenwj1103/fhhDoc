...
def detectRunningExec(jarname):
    ...
    command = "ps -ef | grep -E '%s' |grep -Ev 'grep|start-fhhservice.py|python' " % (jarname)
...
def updateNginxConfig(configpath, localPort, foreignPort, apiPort):
    ...
    data="""
upstream fhh-service-foreign {
    server 127.0.0.1:%s;
    keepalive 15;
}

upstream fhh-service-local {
    server 127.0.0.1:%s;
    keepalive 15;
}

upstream fhh-service-api {
    server 127.0.0.1:%s;
    keepalive 15;
}
...
server {
    ...
    location / {
        proxy_pass http://fhh-service-foreign;
...
server {
    ...
    location / {
        proxy_pass http://fhh-service-local;
        proxy_set_header Connection "Keep-Alive";
        proxy_set_header Proxy-Connection "Keep-Alive";
    }
}
...
server {
    ...
    location / {
        proxy_pass http://fhh-service-api;
        proxy_set_header Connection "Keep-Alive";
        proxy_set_header Proxy-Connection "Keep-Alive";
    }
}
...
if __name__ == '__main__':
    ...
    allRunning = []
    newPorts = {
        "fhh-service-local": 8000,
        "fhh-service-foreign": 8700,
        "fhh-service-api": 9400
    }
    ...
    # nginx
    localPort = newPorts["fhh-service-local"]
    foreignPort = newPorts["fhh-service-foreign"]
    apiPort = newPorts["fhh-service-api"]
